...
async function getEtsyData(kw) {
  try {
    ...
    const L = (data.results || []).filter(l => {
      // Filter to English listings only — remove German/French/Spanish etc
      if (l.language && l.language !== 'en') return false;
      if (l.currency_code && l.currency_code === 'EUR') return false;
      const title = (l.title || '').toLowerCase();
      // Quick language check — German common words
      const nonEnglish = /(und|für|mit|das|die|der|ein|eine|personalisiert|geschenk|tasse|becher)/i;
