...
async function getEtsyData(kw) {
  try {
    ...
    // Add translated_keywords=false to prevent Etsy translating UK terms
    const url = `https://openapi.etsy.com/v3/application/listings/active?keywords=${encodeURIComponent(kw)}&limit=100&sort_on=score&translate_keywords=false`;
    const r = await fetch(url, { headers: { 'x-api-key': header, 'Accept': 'application/json', 'Accept-Language': 'en-GB' } });
    if (!r.ok) return null;
    const data = await r.json();
    
    // Filter: keep only GBP/USD listings, remove EUR (European shops)
    const L = (data.results || []).filter(l => {
      const cc = l.currency_code || '';
      return cc === 'GBP' || cc === 'USD' || cc === '';
    });
    ...
    const count = data.count || L.length;
    const score = count >= 5000 ? 10 : count >= 2000 ? 9 : count >= 1000 ? 8 : count >= 500 ? 7 : count >= 200 ? 6 : count >= 100 ? 5 : count >= 50 ? 4 : count >= 20 ? 3 : 2;
    return { 
      count, 
      avgPrice: avg.toFixed(2), 
      sweetSpot: swLow ? `£${swLow.toFixed(2)}-£${swHigh.toFixed(2)}` : null, 
      topTags, topFav, totalFavs, 
...
export default async function handler(req, res) {
  ...
  // Fetch Etsy data in batches of 5 to avoid rate limiting
  const etsyResults = [];
  const batchSize = 5;
  for (let i = 0; i < allProducts.length; i += batchSize) {
    const batch = allProducts.slice(i, i + batchSize);
    const batchResults = await Promise.allSettled(batch.map(p => getEtsyData(p.kw)));
